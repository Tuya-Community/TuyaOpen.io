...
class BaseDownloader {
  ...
  /**
   * 分包发送大帧，防止 USB 转串口桥接芯片 FIFO 溢出。
   *
   * macOS 上 FlashWrite4K 命令帧为 4108 字节，USB 主机（12Mbps）把数据送入
   * 桥接芯片的速度远快于 UART 输出（约 0.92Mbps@115200），桥接芯片内部 FIFO
   * 约 320 字节，接收约 350 字节后溢出，后续字节丢失/损坏（FlashRead4K 回读
   * 验证：前 344-348 字节正确，之后全部损坏）。
   *
   * 修复参考 tyutool transport.rs send_raw：256 字节分包 + 按当前波特率计算的
   * drain 等待（8N1 每字节 10 bit）。浏览器无法可靠判断 OS 且 Web Serial 缓冲
   * 模型不同，因此无条件分包。调用方负责 getWriter()/releaseLock()。
   *
   * @param {WritableStreamDefaultWriter} writer 已获取的 writer
   * @param {Uint8Array|number[]} data 待发送数据
   * @param {number} baud 当前波特率（用于计算 drain 时长）
   */
  async pacedWrite(writer, data, baud) {
    const u = data instanceof Uint8Array ? data : new Uint8Array(data)
    const CHUNK = 256 // 安全阈值，小于桥接芯片 ~320 字节 FIFO
    if (u.length <= CHUNK) {
      await writer.write(u)
      return
    }
    // 8N1: 每字节 10 bit；256 字节 = 2560 bit。drain = 2560/baud 秒 + 0.5ms 余量
    const drainMs = Math.max(1, Math.ceil((2560 / (baud || 115200)) * 1000 + 0.5))
    for (let i = 0; i < u.length; i += CHUNK) {
      await writer.write(u.slice(i, i + CHUNK))
      await writer.ready // 等待可写（背压/flush），等价于 tyutool 的 io.flush()
