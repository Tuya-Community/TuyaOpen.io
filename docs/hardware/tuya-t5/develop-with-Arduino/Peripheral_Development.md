...
Basic log output example. It uses the `Log` class and the `PR_DEBUG` macro to output debug log information — the fundamental logging usage for all projects.
...
SPI bus communication example. It uses the `SPI` class for SPI data transfer — after initializing the SPI bus, data is sent and return values received via `SPI.transfer()`.
...
I2C master write example. It uses the `Wire` class as an I2C master to send data to a slave device — after initializing the I2C bus, byte data is periodically sent to a specified slave address.
...
- Supports operations such as `open`, `read`, `write`, `lseek`, `readtillN`, `mkdir`, `rename`, and `remove`.
...
Audio recording and playback example. It records an audio clip and immediately plays it back — recording is triggered by a button press, and playback starts automatically upon completion. Audio data is temporarily stored in a memory buffer.
...
Screen color fill example. It uses the `Display` class to fill color blocks on the LCD screen — after initializing the display, the entire screen is filled with random colors at regular intervals.
...
Image display and rotation example. It displays images on the LCD screen and sets the screen rotation direction (0°/90°/180°/270°) using `display.setRotation()`, showing images in different orientations.
...
LVGL graphics library example. It uses LVGL (Light and Versatile Graphics Library) to create a UI on the device — after initializing the LVGL environment, a label widget displaying "Hello World" text is created.
...
BLE GATT server example. It creates a BLE GATT server — defining custom Service UUIDs and Characteristic UUIDs, setting read/write properties and Notify/Indicate notification capabilities, and handling client connection, read/write, and subscription events through callback functions.
...
Basic WiFi TCP client example. It uses `WiFiClient` to establish a TCP connection, send an HTTP GET request to a remote server, and read the response data — the most fundamental network communication example.
...
WiFi Telnet-to-Serial bridge example. It bridges a WiFi Telnet connection with a hardware serial port — after connecting to the device via a Telnet client, data sent and received through Telnet is transparently forwarded to the serial port, and vice versa.
...
WiFi UDP communication example. It uses the `WiFiUDP` class for UDP data exchange — listening on a specified port, printing the source IP, port, and content when a UDP packet is received, and replying with a confirmation message.
...
Tuya IoT quick start example. It demonstrates the complete Tuya IoT device development workflow — configuring the device License, initializing `TuyaIoT`, handling device binding, cloud connection, and DP command receiving through event callbacks, and achieving two-way control between the phone app and the onboard LED.
...
- Demonstrates `TuyaIoT.read()` and `TuyaIoT.write()` for different data types.
...
- Some interfaces (for example, wind scale and AQI ranking) are supported only in the China mainland data center.
...
Publishing messages within an MQTT callback example. It republishes a message inside the message receive callback — subscribing to `inTopic` and forwarding received messages to `outTopic`, implementing a message relay function.
...
Captive Portal example. It creates a WiFi hotspot and starts a DNS server that redirects all domain name resolution requests to the device's own IP address. Combined with an HTTP server, it implements a captive portal page — users see a configuration page automatically upon connecting to the hotspot.
