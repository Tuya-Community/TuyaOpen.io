...
Set a custom font in an LVGL UI on the T5 board — declare it, wire it into the build, and apply it to an object or as the global default. Covers local file-scope use, global registration in `vendor/T5/t5_os/components/lvgl/lvgl_v8/`, and switching `LV_FONT_DEFAULT`.
