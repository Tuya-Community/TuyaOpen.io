...
:::warning
Most peripheral examples ship with T5AI as the default config. When you run `tos.py config choice`, you may not see a pre-built ESP32 config. Select your ESP32 chip manually or create a `.config` file in the example's `config/` directory (see [Adding a New ESP32 Board](esp32-new-board) for the format).
:::
...
This is useful for advanced configuration (partition tables, IDF component settings, verbose builds, etc.). See [`cli_idf.py`](https://github.com/tuya/TuyaOpen/blob/master/tools/cli_command/cli_idf.py) for the implementation.
