...
TuyaOpen on ESP32 uses a dedicated data partition named `"tuya"` for all persistent storage operations via `tkl_flash`. The layout within this partition:

| Offset | Size | Purpose |
|--------|------|---------|
| `0x0000` | 4 KB | Key area |
| `0x1000` | 32 KB | KV data storage (FlashDB/littlefs backed) |
| `0x9000` | 96 KB | UF (user firmware) region |
| Optional | 4 KB | Protected KV (when `KV_PROTECTED_ENABLE`) |

:::note
`tkl_flash_lock()` and `tkl_flash_unlock()` return `OPRT_NOT_SUPPORTED` on ESP32. Flash write protection is not implemented in the TKL adapter. If you need flash protection, use ESP-IDF's flash encryption feature instead (see Firmware Security below).
:::

The `tuya` partition must be present in your ESP-IDF partition table. The default ESP32 platform config includes it. If you customize the partition table via `tos.py idf menuconfig`, ensure the `tuya` data partition remains.
