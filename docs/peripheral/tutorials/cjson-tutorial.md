...
- Your CMake or Kconfig must link the cJSON library (same as other apps that use JSON; see `examples/protocols/http_client` or `apps/tuya_cloud/switch_demo`).
...
Pass `NULL` to `cJSON_InitHooks` to restore default libc `malloc`/`free`. If you switch between PSRAM and internal RAM for different phases, match the allocator to where you free `cJSON_Print` output (see step 4). For background on PSRAM, see [Heap allocation and PSRAM](../memory/heap-allocation-and-psram).

:::note
`cJSON_InitHooks` affects the global allocator choice for cJSON in that process; call it before any other cJSON API if you rely on it.
:::
...
- `cJSON_Parse*` → you own the tree → call `cJSON_Delete(root)` when done.
- `cJSON_Print*` → you own the returned `char *` → free it once with the matching allocator.
