---
title: "固件升级"
description: "固件 OTA 升级通过涂鸦 IoT 平台远程推送新版本固件到已部署设备，涵盖创建固件 key、上传、上架、配置与发布七步流程。"
keywords:
  - TuyaOpen
  - 固件升级
  - OTA
...
固件 OTA（空中下载）升级通过涂鸦 IoT 平台将新版本固件远程推送到已部署的设备上。本文将带你完成七个步骤：创建固件 key、将 key 写入固件、上传新版本、固件上架、配置升级规则、验证、发布。

## 前提条件

- 已在 [涂鸦 IoT 平台](https://iot.tuya.com) [创建产品](https://developer.tuya.com/cn/docs/iot/create-product?id=K914jp1ijtsfe)，拿到 PID。
...
3. 填写版本号和版本说明，将编译生成的固件文件逐一上传。TuyaOpen 固件包含以下三种类型，可按文件名区分：
