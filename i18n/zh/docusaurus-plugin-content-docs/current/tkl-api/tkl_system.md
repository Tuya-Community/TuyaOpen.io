...
- 返回值：`SYS_TIME_T` 为从系统启动到当前时刻的总毫秒数。
...
- 参数：`range` 为随机数的范围，从 `0` 到 `range`（不包括 `range` 本身）。
...
- 参数：`describe` 为指向描述系统复位原因的字符串的指针。

- 返回值：`TUYA_RESET_REASON_E` 为复位原因的枚举值。
