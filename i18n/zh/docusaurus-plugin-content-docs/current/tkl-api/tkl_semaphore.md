...
返回值为 `OPRT_OK` 则表示成功创建信号量，其余返回值表示发生错误，详细错误代码请参考 `tuya_error_code.h`。
...
`OPRT_OK` 则表示成功获取信号量，`OPRT_OS_ADAPTER_SEM_WAIT_TIMEOUT` 表示发生超时，其他返回值表示发生错误，详细错误代码请参考 `tuya_error_code.h`。
...
`OPRT_OK` 则表示成功发送信号量，其它返回值表示发生错误，详细错误代码请参考 `tuya_error_code.h`。
...
`OPRT_OK` 则表示成功释放资源，其他返回值表示发生错误，详细错误代码请参考 `tuya_error_code.h`。
