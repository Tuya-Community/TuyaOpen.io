...
按键驱动框架在各平台上遵循相同的生命周期。以 GPIO 按键为例：板级先注册驱动，应用再创建按键实例、绑定事件回调，使用完毕后删除实例。
...
1. **注册** — 板级调用 `tdd_gpio_button_register`，传入 `BUTTON_NAME` 和 `BUTTON_GPIO_CFG_T` 配置（`pin`、`mode`、`level`、`pin_type`）。TDD 驱动调用 `tdl_button_register` 注册其 `TDL_BUTTON_CTRL_INFO` 接口，并在设备列表中创建按键节点（`TDL_BUTTON_LIST_NODE_T`）。
2. **创建** — `tdl_button_create` 传入 `TDL_BUTTON_CFG_T` 配置（消抖、长按、多击等时间参数），调用 `__tdd_create_gpio_button` 初始化 GPIO、配置引脚模式，并在中断模式下启用中断。随后 TDL 层启动扫描任务和状态机并返回句柄。
3. **注册回调** — `tdl_button_event_register` 将各事件类型的回调存入事件数组。
4. **扫描与分发** — 扫描任务（或中断）调用 `__tdd_read_gpio_value` 读取引脚状态；状态机（`__tdl_button_state_handle`）完成消抖和事件检测，并对检测到的事件执行已注册的回调。
5. **删除** — `tdl_button_delete` 调用 `__tdd_delete_gpio_button` 禁用中断并释放 GPIO 资源，移除节点，若为最后一个按键则停止扫描任务。
