...
音频连接框架取决于主控芯片。`T5AI` 自带 ADC 和 DAC 接口，无需 CODEC 芯片即可搭建音频系统；`ESP32-S3` 不支持 DAC，需要外挂 CODEC 芯片。

### 内置 CODEC

当 MCU 自带 ADC 和 DAC 时，无需 CODEC 芯片，信号通路如下：

| 环节 | 通路 |
| --- | --- |
| 采集 | 麦克风 → MCU ADC |
| 播放 | MCU DAC → PA 输入 → PA 输出 → 扬声器 |
| 功放控制 | MCU `PA_GPIO` → PA `CTRL` |

### 外置 CODEC

当 MCU 不带 DAC 时（例如 `ESP32-S3`），由外置 CODEC 芯片完成转换。MCU 通过两条总线驱动 CODEC：`I2C` 传输控制和配置信号，`I2S` 传输 PCM 音频数据。信号通路如下：

![外置 Codec 音频通路：采集为 麦克风 → Codec ADC → I2S → MCU；播放为 MCU → I2S → Codec DAC → PA → 扬声器，并带 I2C 控制和 PA_GPIO 使能线](/f2b18334-387d-42f3-b39b-c85b6916ff76.png)

| 环节 | 通路 |
| --- | --- |
| 控制 | MCU `I2C` ↔ CODEC `I2C` |
| 采集 | 麦克风 → CODEC ADC → CODEC `I2S` → MCU `I2S` |
| 播放 | MCU `I2S` → CODEC `I2S` → CODEC DAC → PA 输入 → PA 输出 → 扬声器 |
| 功放控制 | MCU `PA_GPIO` → PA `CTRL` |
...
音频驱动框架在各平台上遵循相同的生命周期。以 T5AI 为例：板级先注册设备，应用再按名称查找设备、打开、播放或采集音频，最后关闭。

```mermaid
sequenceDiagram
    participant App as 应用
    participant TDL as TDL（管理层）
    participant TDD as TDD 驱动
...
完整流程还包含注册、配置、音量和停止等步骤：

1. **注册** — 板级调用 `tdd_audio_register`，传入 `AUDIO_CODEC_NAME` 和 `TDD_AUDIO_T5AI_T` 配置（`ai_chn`、`sample_rate`、`data_bits`、`channel`、`spk_pin`、`aec_enable`）。TDD 驱动随后调用 `tdl_audio_driver_register` 注册其 `TDD_AUDIO_INTFS_T` 接口，并在设备列表中创建音频节点（`TDL_AUDIO_NODE_T`）。
2. **查找** — 应用调用 `tdl_audio_find`，按名称查找设备并获取句柄。
3. **打开** — `tdl_audio_open` 调用 `__tdd_audio_open`，初始化硬件、配置 ADC/DAC 与 I2S 接口，并设置扬声器使能引脚。
4. **配置** — 配置接口调用 `__tdd_audio_config`，设置采样率、数据位数、声道，并在启用时配置回声消除。
5. **设置音量** — `tdl_audio_volume_set` 调用 `__tdd_audio_set_volume`，调整输出音量与功放增益。
6. **播放** — `tdl_audio_play` 将帧数据和帧格式传给 `__tdd_audio_play`，由其输出到 DAC 或 I2S 并控制扬声器使能。
7. **停止与关闭** — `tdl_audio_play_stop` 停止输出；`tdl_audio_close` 调用 `__tdd_audio_close` 去初始化硬件并释放 I2S 资源。
...
![open_audio](/58c12df0-250d-48be-8880-be8f50623451.png)
...
1. 根据音频 CODEC 的型号与连接引脚编写注册接口。建议写在 `board_register_hardware()` 中，该接口实现的路径为 `boards/<target_platform>/<target_board>/xxx.c`。
