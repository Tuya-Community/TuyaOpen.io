...
本文介绍如何为 TuyaOpenClaw（原名 DuckyClaw）创建自定义设备 MCP 工具（硬件技能）。面向希望将摄像头、传感器、显示屏等外设作为技能接入同一套 Agent 框架的应用开发者——使智能体能够基于传感器数据推理、在硬件上触发动作，并将设备输入与云端 AI 结合。

## 前置条件

- 已针对目标平台（Tuya T5AI、ESP32、Raspberry Pi 或 Linux）完成[环境配置](/docs/quick-start/enviroment-setup)。
- 对 [MCP Server](/docs/cloud/device-ai/ai-components/ai-mcp-server)（工具发现与执行）与 [MCP Tools](/docs/cloud/device-ai/ai-components/ai-mcp-tools)（预定义设备工具）有基本了解。
- 具备可运行 TuyaOpenClaw 的开发板或运行环境。支持的平台见 [TuyaOpenClaw 项目](https://github.com/tuya/DuckyClaw)。
...
## 概述：TuyaOpenClaw 中的设备 MCP

在 TuyaOpenClaw 中，**设备 MCP** 指设备侧运行 MCP 服务器并对外暴露**工具**，供本地或云端智能体发现与调用。每个工具包含名称、描述、输入参数及在设备上执行的回调。TuyaOpenClaw 内置的设备 MCP 工具包括：
...
在配置中确保已启用 MCP 组件（如 `ai_mcp/Kconfig` → `ENABLE_COMP_AI_MCP`）。在应用启动时，待 MQTT（或 TuyaOpenClaw 使用的传输通道）就绪后，初始化 MCP 服务器并注册你的工具。
...
详见：[MCP Server – 初始化 MCP 服务器](/docs/cloud/device-ai/ai-components/ai-mcp-server#initialize-mcp-server)。
...
参考：[MCP Server – 属性定义宏](/docs/cloud/device-ai/ai-components/ai-mcp-server#attribute-definition-macro)与[开发步骤](/docs/cloud/device-ai/ai-components/ai-mcp-server#development-steps)。

### 4. 接入消息处理

确保将收到的 MCP 消息（如通过 TuyaOpenClaw 与智能体共用的通道传输的 JSON-RPC）交给服务器处理：

```c
ai_mcp_server_parse_message(json, NULL);
```

通常在 TuyaOpenClaw/AI 智能体栈的消息或 MQTT 回调中调用。服务器会分发到对应工具回调并返回响应。

### 5. 构建与验证

针对目标板构建应用，烧录或部署后运行 TuyaOpenClaw。确认智能体（或云端）能列出并调用新工具。预期结果：工具出现在工具列表中，被调用时返回正确类型的值（整数、字符串、JSON 等）。

## 与 TuyaOpen 驱动的集成

硬件技能应尽量使用 TuyaOpen 驱动与 API，使工具回调在不同开发板上保持可移植。通过 TKL 或驱动层访问外设（如传感器的 ADC、继电器的 GPIO），避免板级专用代码。这与 [TuyaOpenClaw 页面](/duckyclaw) 中「基于 TuyaOpen 驱动与 API 快速原型开发」的做法一致。
...
- [MCP Server](/docs/cloud/device-ai/ai-components/ai-mcp-server) — 设备 MCP 接入、工具注册与 API 说明。
- [MCP Tools](/docs/cloud/device-ai/ai-components/ai-mcp-tools) — 预定义设备工具与初始化。
- [通用示例](/docs/examples/demo-generic-examples) — 验证硬件接口及最小接口代码示例。
- [TuyaOpenClaw 项目](https://github.com/tuya/DuckyClaw) — 仓库与内置设备 MCP 工具（CRON、FILE、IoT、EXEC）。
