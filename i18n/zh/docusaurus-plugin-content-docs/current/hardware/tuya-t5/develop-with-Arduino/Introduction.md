...
Arduino-TuyaOpen 基于 [TuyaOpen](https://github.com/tuya/TuyaOpen) 框架二次开发，提供了更简洁的 Arduino 风格 API，方便快速开发。它基于 Arduino 通用接口标准设计，支持 Bluetooth、Wi-Fi、Ethernet 等通信协议，提供物联网开发的核心能力，包括配网、激活、控制、OTA 升级等；它具备设备认证、数据加密、通信加密等安全合规能力，满足全球各国家和地区的数据合规需求。

基于 TuyaOpen 开发的 AIoT 产品，可以使用涂鸦 App 和云服务提供的强大生态能力，并与所有 Powered by Tuya 设备互联互通。

TuyaOpen 持续拓展，提供更多云平台接入能力，以及语音、视频、AI Agent 功能，Arduino-TuyaOpen 也会同步更新支持这些新能力。
...
- **模拟输入**：`analogRead()` —— 读取传感器数值（温湿度、光照、电位器等），12 位精度。
...
- **Wi-Fi 连接**：支持 AP 配网与蓝牙配网，连接涂鸦云实现远程控制。
...
- **场景联动**：与其他 Powered by Tuya 设备联动，构建智能家居场景。
...
- **智能传感器节点**：使用 `analogRead()` 读取温湿度传感器，通过 Wi-Fi 上传数据到涂鸦云，在 App 中查看历史曲线并接收异常告警。
...
- [快速开始](Quick_start.md) —— 安装 Arduino IDE 并烧录第一个示例。
