...
关于 TKL 层适配，可参考 [补充代码](./new-platform.md#%E8%A1%A5%E5%85%85%E4%BB%A3%E7%A0%81) 及 [移植 TuyaOS 到 RTOS 平台](https://developer.tuya.com/cn/docs/iot-device-dev/TuyaOS-translation_rtos?id=Kcrwraf21847l) 文档进行适配和完善。需要注意，原厂 SDK 可能已集成 lwip 和 mbedtls，可根据实际情况选择使用原厂或 TuyaOpen 的版本。
...
关于创建 Board 的操作步骤，可参考 [创建 Board](./new-board.md) 章节。


Board 创建完成后，经过 [调整配置](./new-board.md#%E8%B0%83%E6%95%B4%E9%85%8D%E7%BD%AE) 和 [保存配置](./new-board.md#%E4%BF%9D%E5%AD%98%E9%85%8D%E7%BD%AE) 后，即可在 `apps/tuya_cloud/switch_demo` 目录下通过 `tos.py config choice` 命令选择对应开发板，随后进入下一步验证功能。
