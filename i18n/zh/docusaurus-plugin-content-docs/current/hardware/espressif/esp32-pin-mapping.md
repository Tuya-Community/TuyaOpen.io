...
各芯片可用的 GPIO 范围见下文 [各平台引脚映射文档](#各平台引脚映射文档)。

### Pinmux（引脚复用）

**作用**：ESP32 片上外设（I2C、PWM、UART 等）并不固定绑定某几根引脚，而是可以灵活路由到不同的 GPIO。TKL 的 `tkl_io_pinmux_config()` 封装了这一能力，让开发者在外设初始化前指定实际使用的引脚。
...
ESP32 上的 ADC 在 [`tkl_adc.c`](https://github.com/tuya/TuyaOpen-esp32/blob/master/tuya_open_sdk/tuyaos_adapter/src/drivers/tkl_adc.c) 中采用 **端口 + 通道位掩码** 模型：

| TuyaOpen 端口 | ESP-IDF 单元 | 说明 |
|--------------|-------------|------|
| `TUYA_ADC_NUM_0` | `ADC_UNIT_1` | 始终可用 |
| `TUYA_ADC_NUM_1` | `ADC_UNIT_2` | 在经典 ESP32 上 Wi-Fi 工作期间不可用 |

**通道选择**：`cfg->ch_list.data` 是一个位掩码，第 N 位置位即启用 `ADC_CHANNEL_N`。

**固定配置**：衰减为 `ADC_ATTEN_DB_12`（约 0–3.3 V 范围）。`tkl_adc_ref_voltage_get()` 返回 3300 mV。校准在 S2/S3/C3/C6 上采用曲线拟合（curve-fitting），在经典 ESP32 上采用线性拟合（line-fitting）。

**不支持**：`tkl_adc_temperature_get()` 返回 `OPRT_NOT_SUPPORTED`，请改用 ESP-IDF 的温度传感器驱动。
