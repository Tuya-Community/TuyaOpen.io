...
:::tip[命名规范]
板子名称建议使用全大写字母和下划线，格式为 `{厂商}_{芯片}_{型号}`，例如 `WAVESHARE_ESP32S3_TOUCH_AMOLED`。名称会同时作为目录名和 Kconfig 中 `BOARD_CHOICE` 的值，两者必须完全一致（大小写敏感）。更详细的规范请参考 [创建 Board](../../hardware/porting/new-board)。
:::
...
- [创建 Board](../../hardware/porting/new-board)
