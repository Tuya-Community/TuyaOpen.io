...
* **主代码库**（含核心逻辑与驱动）
  ...
  [TuyaOpen/apps/tuya.ai/your_otto_robot/src/otto_ninja](https://github.com/tuya/TuyaOpen/tree/master/apps/tuya.ai/your_otto_robot/src/otto_ninja)
...
* **原理图与 PCB 设计**（立创 EDA 平台）
  ...
  [Otto Ninja 硬件项目](https://oshwhub.com/robben.wang/ottorobot_ninja)

:::note
包含主控模块、舵机接口、显示屏接口、电源管理模块的完整设计，支持直接导出生产文件。
:::
...
* **模型仓库地址**
  ...
  [TuyaOpen/apps/tuya.ai/your_otto_robot/src/otto_ninja/3D_Models](https://github.com/tuya/TuyaOpen/tree/master/apps/tuya.ai/your_otto_robot/src/otto_ninja)（建议进入后跳转至 “3D_Models” 子目录下载）
...
| ----------- | ------------------------------------------- | ---- | ------------------------------------------------------------ | ------------------------------------ |
| 橡胶密封圈  | O 型圈、70*5(5个）                          | 按需 | [淘宝链接](https://e.tb.cn/h.SwzLdCs7xnQF7FB?tk=ajRffNdG3MQ) | 用于轮足                             |
...
| 麦克风模块  | 6027 型号、模拟信号、1.25mm 端子、IP65 防水 | 1 个 | [淘宝链接](https://item.taobao.com/item.htm?id=764269727410) | 用于语音交互功能                     |
| 主控板      | T5 OTTO开发板/整套材料                      | 1 块 | [淘宝链接](https://tuyasmart.taobao.com/?spm=a1z10.5-c-s.0.0.223d5cb0RYr5fX) | 整体材料                             |

:::note
物料数量可根据打印模型版本调整，建议首次制作按 “1 套” 采购，预留 1-2 个易损件（如舵机、密封圈）。
:::
...
* **组装教程**：参考视频演示 [Otto Ninja 安装演示教程](https://b23.tv/dKEJ69n)（关键步骤：舵机校准→四肢组装→主控板固定→显示屏安装）；
...
1. 下载并安装 [Tyutool 工具（Windows 版）](https://developer.tuya.com/cn/docs/developer/windows_tyutool_gui?id=Kew1ks0xuct2p)；
