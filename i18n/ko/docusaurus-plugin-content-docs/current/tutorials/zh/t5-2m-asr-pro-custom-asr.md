...
UART에서 ASR PRO 음성 인식 모듈을 첨부하여 T5 개발 보드에 사용자 정의 웨이브 단어를 추가하십시오. - 오디오 알고리즘이 필요하지 않으며 각 웨이브 단어는 다른 Tuya AI 에이전트 역할로지도 할 수 있습니다. T5 플랫폼은 기본 깨진 단어의 고정 세트로 배송됩니다.
- TuyaOS는 "Nihao Tuya"(Hello Tuya), "Xiao Zhi Tongxue"(클래스 Zhi), "Hey, Tuya"(영어)의 세 가지 기본 웨이브 단어를 지원합니다.
- TuyaOpen은 4 개의 기본 깨어있는 단어를 지원합니다. "Nihao Tuya", "Xiao Zhi Tongxue", "Xiao Zhi Guanjia"(버틀러 Zhi) 및 "Nihao Xiao Zhi"

기본을 넘어 가기 위해, 옵션은:
- 음성 인식 모듈 추가
- [Wake word 알고리즘을 대체합니다.](https://developer.tuya.com/cn/docs/iot-device-dev/tuyaos-wukong-capability-wakeup-internal?id=Keia2dqx0v23b#title-3-%E6%9B%BF%E6%8D%A2%E5%94%A4%E9%86%92%E7%AE%97%E6%B3%95)
- 사용자 정의 웨이브 단어 (사용자 정의, Tuya 판매에 문의하시기 바랍니다)

이 튜토리얼은 첫 번째 경로가 걸립니다 - 다음 장점을 위해 ASR PRO 모듈을 추가하십시오.

- 낮은 기술 장벽, 오디오 알고리즘을 이해 할 필요가 없습니다.
- Tuya AI Agent Multi-role 기능으로 여러 가지 웨이브 단어를 지원합니다. 다른 웨이브 단어는 다른 역할에 대응할 수 있으며 역할과 깨어있는 단어 사이에 어색한 mismatches를 피할 수 있습니다.
- 오프라인 음성 인식 지원 ASR PRO 모듈은 오프라인 중립 네트워크 컴퓨팅을 지원하며 네트워크에 의존하지 않고 음성 인식을 가능하게 합니다. 이 정상적인 가동은 아니 또는 불안정한 네트워크 연결을 가진 환경에서 조차 지킵니다.
- 높은 승인 비율. ASR PRO 모듈은 강한 에코 취소 및 환경 소음 억제를 특징으로하며, 소음 환경에서도 높은 음성 인식 정확도를 유지합니다. 승인 비율은 98% 이상, 0.15 초 미만의 응답 시간으로 도달할 수 있습니다.

## 관련 자료

|제품정보|기능 / 모수|
|------------------------------------|-------------------------------------------------------------------------------------|
|T5 개발위원회|핵심 AI 대화 기능 제공|
|ASR PRO 모듈|사용자 정의 깨진 단어 및 오프라인 음성 인식 지원|
|직렬 보드 또는 ASR-LINK Downloader|표준 CH340 직렬 보드는 ASR PRO에 다운로드 프로그램에 충분합니다. ASR-LINK 자동 다운로더를 구입할 수 있습니다.|
|마이크|ASR PRO 모듈의 오디오 픽업에 사용|


## 2M ASR PRO 음성 인식 모듈 (Core Board)
칩 자료표:[더 보기](https://www.haohaodada.com/jpeguploadfile/twen/ASRPRO/ASRPROCoreV1.1.pdf)
...
먼저 ASR PRO에 프로그래밍 및 플래시 펌웨어가 필요하며, T5 개발 보드에 ASR PRO 모듈을 연결합니다. 따라서, 배선은 2개 부품으로 분할됩니다:
...
## ASR PRO 프로그램 개발
토야 AI 에이전트는 여러 역할을 지원하며, ASR PRO 모듈은 최대 5개의 웨이브 단어를 지원하며, 고유한 인식 ID로 각각 지원합니다. 각 역할에는 전용 웨이브 단어가 있습니다. ASR PRO 프로그램을 개발할 때, 예를 들어, 다른 IO 핀을 제어하거나 시리얼을 메인 컨트롤러 T5로 전송하는 인식 ID를 사용할 수 있습니다.

ASR PRO 모듈 개발을 위해 **Tianwen Block**를 사용하는 것이 좋습니다. 그래픽 프로그래밍 인터페이스는 매우 초보자 친화적입니다. 다운로드 할 수 있습니다.[Tianwen 51 공식 웹 사이트](http://twen51.com/new/twen51/index.php).

### 예제 프로그램

시스템은 세 가지 웨이브 단어로 구성되어 있습니다 : "Hello Nezha", "Hello Zhu Bajie", "Hello Conan". ASR PRO 모듈이 이러한 웨이브 단어 중 하나를 인식 할 때, 시리얼을 통해 IO 및 출력 데이터를 통해 깨진 기능을 트리거해야합니다.
- 깨진 단어를 인식 한 후, ASR PRO는 PA4을 100 ms로 낮게 설정 한 후, 높이를 끌어냅니다.
...
- 회사:[다운로드](https://images.tuyacn.com/content-platform/hestia/content-platform/hestia/1756434287f142a5459ac.zip)
- Flashing 공구:[다운로드](https://images.tuyacn.com/content-platform/hestia/content-platform/hestia/content-platform/hestia/175643445346d29fa1f7a.zip)

### 회사 소개

펌웨어 번쩍기에 대한 지침은 참조하시기 바랍니다[비디오 자습서](https://haohaodada.com/new/tencentCloud/cloudiframe.php?id=146).
...
## T5 프로그램 개발
### IO 웨이크업
IO Wakeup은 상대적으로 간단합니다. 그냥 T5의 웨이브 핀 (기본 P12)에 ASR PRO의 웨이브 핀을 연결.

### 연락처

serial Wakeup의 경우, 직렬 수신 프로그램에서 인식 된 Wake 명령을 확인하고 호출합니다.`audio_recorder_start();`인터페이스. 아래는 serial 예제입니다.
