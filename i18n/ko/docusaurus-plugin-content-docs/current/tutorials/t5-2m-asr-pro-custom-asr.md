...
## 프로젝트 소개
ASR PRO 연설 인식 모듈을 UART에 첨부하여 T5 개발 보드에 사용자 정의 웨이브 단어를 추가하십시오. - 오디오 알고리즘이 필요하지 않으며 각 웨이브 단어는 다른 Tuya AI Agent 역할로지도 할 수 있습니다. T5 플랫폼은 기본 깨진 단어의 고정 세트로 배송됩니다.
...
이 튜토리얼은 첫 번째 경로가 걸립니다 - ASR PRO 모듈을 추가 - 다음 장점 :
