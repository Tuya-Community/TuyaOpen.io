...
## 프로젝트 개요
IoT Switch 프로젝트는 Tuya의 플랫폼을 사용하여 클라우드 연결 장치를 구축하는 방법의 단순하면서 강력한 데모입니다. 이 예제는 모바일 앱을 통해 제어 할 수있는 기본 스위치를 보여줍니다. DP (Data Point) 제어 및 연결 장치에 대한 최소 코드 구현과 같은 필수 개념을 강조. 이 프로젝트는 PID, UUID 및 Auth Key를 사용하여 안전한 장치 인증을 포함하고 Tuya T 시리즈 MCUs, ESP32 및 Linux 기반 MCU를 포함한 다양한 하드웨어 플랫폼과 호환됩니다.
