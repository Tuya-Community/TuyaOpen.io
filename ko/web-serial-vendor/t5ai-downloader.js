...
class T5Downloader extends BaseDownloader {
  ...
  async sendCommand(command, commandName) {
    ...
    try {
      ...
      // 分包发送，防止 Mac 平台 USB 转串口桥接芯片 FIFO 溢出（FlashWrite4K 4108 字节帧）
      await this.pacedWrite(writer, command, this.currentBaudrate || 115200)
    } catch (error) {
      ...
      while (responseBuffer.length < expectedLength && Date.now() - startTime < timeout && !this.stopFlag) {
        ...
        try {
          ...
          if (result.value && result.value.length > 0) {
            ...
            this.debugLog(
              `接收: ${Array.from(result.value)
                .map((b) => b.toString(16).padStart(2, '0').toUpperCase())
                .join(' ')} (累计${responseBuffer.length}字节)`,
            )
  ...
  async getChipId() {
    ...
    if (response.length >= 15) {
      ...
      if (r[0] === 0x04 && r[1] === 0x0e && r[3] === 0x01 && r[4] === 0xe0 && r[5] === 0xfc && r[6] === 0x03) {
        ...
        if (r[9] === 0x0e) {
          ...
          if (status === 0x00) {
            ...
            if (response.length >= 15) {
              ...
              this.debug(
                'debug',
                `Flash ID原始数据: ${flashIdData.map((b) => b.toString(16).padStart(2, '0').toUpperCase()).join(' ')}`,
              )
            ...
            throw new Error(
              `Flash操作失败: ${errorDesc} (状态码: 0x${status.toString(16).padStart(2, '0').toUpperCase()})`,
            )
          }
        } else {
  ...
  async connect() {
    try {
      ...
      if (this.flashConfig) {
        this.infoLog(
          `Flash型号: ${this.flashConfig.manufacturer} ${this.flashConfig.name} (${this.flashConfig.size / 1048576}MB)`,
        )
      }
  ...
  async downloadFirmware(fileData, startAddr = 0x00) {
    ...
    try {
      ...
      this.infoLog(
        `目标Flash: ${this.flashConfig ? this.flashConfig.manufacturer + ' ' + this.flashConfig.name : '未知型号'}`,
      )
      ...
      if (response[0] === 0x04 && response[1] === 0x0e) {
        ...
        if (response[2] === expectedLength) {
          ...
          if (response[3] === 0x01 && response[4] === 0xe0 && response[5] === 0xfc) {
            ...
            this.errorLog(
              `TX头部错误: 期望[01 E0 FC], 实际[${response
                .slice(3, 6)
                .map((b) => b.toString(16).padStart(2, '0').toUpperCase())
                .join(' ')}]`,
            )
  ...
  async unprotectFlash() {
    ...
    if (this.compareRegisterValue(regVal, unprotectRegVal, mask)) {
      ...
      for (let i = 0; i < regVal.length && i < unprotectRegVal.length && i < mask.length; i++) {
        ...
        this.debug(
          'debug',
          `寄存器${i}: 读取值=0x${regVal[i].toString(16).padStart(2, '0')}, 掩码=0x${mask[i].toString(16).padStart(2, '0')}, 读取值&掩码=0x${srcMasked.toString(16).padStart(2, '0')}, 目标值&掩码=0x${destMasked.toString(16).padStart(2, '0')}, 匹配=${srcMasked === destMasked ? '是' : '否'}`,
        )
      }
      ...
      for (let i = 0; i < writeVal.length; i++) {
        ...
        this.debug(
          'debug',
          `计算写入值${i}: 目标=0x${unprotectRegVal[i].toString(16).padStart(2, '0')}, 反掩码=0x${invertedMask.toString(16).padStart(2, '0')}, 保留位=0x${preserved.toString(16).padStart(2, '0')}, 最终写入=0x${writeVal[i].toString(16).padStart(2, '0')}`,
        )
      }
      ...
      this.debug(
        'debug',
        `写入后状态寄存器值: [${newRegVal.map((v) => '0x' + v.toString(16).padStart(2, '0')).join(', ')}]`,
      )
      ...
      if (this.compareRegisterValue(newRegVal, unprotectRegVal, mask)) {
        ...
        for (let i = 0; i < newRegVal.length && i < unprotectRegVal.length && i < mask.length; i++) {
          ...
          this.debug(
            'error',
            `验证失败 - 寄存器${i}: 写入后值=0x${newRegVal[i].toString(16).padStart(2, '0')}, 掩码=0x${mask[i].toString(16).padStart(2, '0')}, 实际&掩码=0x${srcMasked.toString(16).padStart(2, '0')}, 期望&掩码=0x${destMasked.toString(16).padStart(2, '0')}`,
          )
        }
  ...
  async readFlashStatusRegVal(retry = 5) {
    ...
    for (let regIndex = 0; regIndex < readRegCode.length; regIndex++) {
      ...
      for (let retryCount = 0; retryCount < retry; retryCount++) {
        try {
          ...
          this.debug(
            'debug',
            `发送读取寄存器${tmpReg}命令: ${command.map((b) => b.toString(16).padStart(2, '0').toUpperCase()).join(' ')}`,
          )
          ...
          this.debug(
            'debug',
            `读取寄存器${tmpReg}响应: 长度=${response.length}, 数据=${response.map((b) => b.toString(16).padStart(2, '0').toUpperCase()).join(' ')}`,
          )
            ...
            if (headerMatch) {
              ...
              if (response[10] === 0x00) {
                ...
                if (response[11] === tmpReg) {
                  ...
                  this.debug(
                    'warning',
                    `寄存器${tmpReg}地址回显错误: 期望0x${tmpReg.toString(16).padStart(2, '0')}, 实际0x${response[11].toString(16).padStart(2, '0')}，重试...`,
                  )
                }
              } else {
                this.debug(
                  'warning',
                  `寄存器${tmpReg}状态码错误: 0x${response[10].toString(16).padStart(2, '0')}，重试...`,
                )
              }
            } else {
              this.debug(
                'warning',
                `寄存器${tmpReg}响应头部错误: 期望[${expectedHeader.map((b) => '0x' + b.toString(16).padStart(2, '0')).join(', ')}], 实际[${response
                  .slice(0, 7)
    ...
    this.debug(
      'debug',
      `Flash状态寄存器读取完成: [${srVal.map((v) => '0x' + v.toString(16).padStart(2, '0')).join(', ')}]`,
    )
    return srVal
  }
  ...
  async writeFlashStatusRegVal(writeVal, retry = 5) {
    ...
    this.debug(
      'debug',
      `开始写入Flash状态寄存器，寄存器代码: [${writeRegCode.join(', ')}], 写入值: [${writeVal.map((v) => '0x' + v.toString(16).padStart(2, '0')).join(', ')}]`,
    )
    ...
    if (writeRegCode.length === 1) {
      ...
      for (let retryCount = 0; retryCount < retry; retryCount++) {
        try {
          ...
          this.debug(
            'debug',
            `发送写入寄存器${regAddr}命令: ${command.map((b) => b.toString(16).padStart(2, '0').toUpperCase()).join(' ')}`,
          )
          ...
          this.debug(
            'debug',
            `写入寄存器${regAddr}响应: 长度=${response.length}, 数据=${response.map((b) => b.toString(16).padStart(2, '0').toUpperCase()).join(' ')}`,
          )
  ...
  async eraseCustomSize(addr, eraseCmd) {
    ...
    this.debug(
      'debug',
      `擦除命令详情: 地址=0x${addr.toString(16).padStart(8, '0')}, 命令=0x${eraseCmd.toString(16).padStart(2, '0')}`,
    )
    ...
    this.debug(
      'debug',
      `擦除响应: 长度=${response.length}, 数据=${response.map((b) => b.toString(16).padStart(2, '0').toUpperCase()).join(' ')}`,
    )
        ...
        if (response[7] === expectedLengthBytes[0] && response[8] === expectedLengthBytes[1]) {
          ...
          if (response[10] === 0x00) {
            ...
            if (response[9] === 0x0f && response[11] === eraseCmd) {
              ...
              const addrBytes = [addr & 0xff, (addr >> 8) & 0xff, (addr >> 16) & 0xff, (addr >> 24) & 0xff]
              ...
              if (response.slice(12, 16).every((byte, index) => byte === addrBytes[index])) {
                ...
                throw new Error(
                  `擦除响应地址不匹配: 期望[${addrBytes.map((b) => '0x' + b.toString(16).padStart(2, '0')).join(', ')}], 实际[${response
                    .slice(12, 16)
                    .map((b) => '0x' + b.toString(16).padStart(2, '0'))
                    .join(', ')}]`,
                )
              ...
              throw new Error(
                `擦除响应命令码错误: 期望0x0f和0x${eraseCmd.toString(16)}, 实际0x${response[9].toString(16)}和0x${response[11].toString(16)}`,
              )
            }
          } else {
          ...
          throw new Error(
            `擦除响应长度字段错误: 期望[${expectedLengthBytes.map((b) => '0x' + b.toString(16).padStart(2, '0')).join(', ')}], 实际[0x${response[7].toString(16).padStart(2, '0')}, 0x${response[8].toString(16).padStart(2, '0')}]`,
          )
        }
      } else {
        throw new Error(
          `擦除响应头部格式错误: 期望[${expectedHeader.map((b) => '0x' + b.toString(16).padStart(2, '0')).join(', ')}], 实际[${response
            .slice(0, 7)
            .map((b) => '0x' + b.toString(16).padStart(2, '0'))
            .join(', ')}]`,
        )
  ...
  async writeSector(flashAddr, buf, flashSize) {
    ...
    const addrBytes = [flashAddr & 0xff, (flashAddr >> 8) & 0xff, (flashAddr >> 16) & 0xff, (flashAddr >> 24) & 0xff]
    ...
    if (response.length >= expectedLength) {
      ...
      if (!headerMatch) {
        this.debug(
          'error',
          `响应头部不匹配: 期望[${expectedBaseHeader.map((b) => b.toString(16).padStart(2, '0').toUpperCase()).join(' ')}]`,
        )
        this.debug(
          'error',
          `实际收到[${response
      ...
      if (isExt) {
        ...
        if (responseAddr.every((byte, index) => byte === addrBytes[index])) {
          ...
          this.debug(
            'error',
            `响应地址不匹配: 期望[${addrBytes.map((b) => b.toString(16).padStart(2, '0').toUpperCase()).join(' ')}]`,
          )
          this.debug(
            'error',
            `实际收到[${responseAddr.map((b) => b.toString(16).padStart(2, '0').toUpperCase()).join(' ')}]`,
          )
        ...
        if (
          ...
          if (isExt) {
            ...
            if (crcMe === crcRead) {
              this.debug(
                'debug',
                `✅ CRC校验成功 (扩展协议): 计算值=0x${crcMe.toString(16)}, 读取值=0x${crcRead.toString(16)}`,
              )
              return true
            } else {
              this.debug(
                'warning',
                `CRC不匹配 (扩展协议): 计算值0x${crcMe.toString(16)}, 读取值0x${crcRead.toString(16)}`,
              )
            }
          } else {
            // 普通协议需要完整的响应检查
            ...
            if (response[6] === cmd) {
              ...
              if (crcMe === crcRead) {
                this.debug(
                  'debug',
                  `✅ CRC校验成功 (普通协议): 计算值=0x${crcMe.toString(16)}, 读取值=0x${crcRead.toString(16)}`,
                )
                return true
              } else {
                this.debug(
                  'warning',
                  `CRC不匹配 (普通协议): 计算值0x${crcMe.toString(16)}, 读取值0x${crcRead.toString(16)}`,
                )
              }
            } else {
              this.debug(
                'warning',
                `CRC响应命令码错误: 期望0x${cmd.toString(16)}, 实际0x${response[6]?.toString(16) || 'unknown'}`,
              )
            }
          }
        } else {
  ...
  async readSector(flashAddr, flashSize) {
    ...
    if (response.length >= expectedLength) {
      ...
      if (headerMatch && response[10] === 0x00) {
        // 检查状态码
        if (isExt) {
          ...
          if (responseAddr.every((byte, index) => byte === expectedAddr[index])) {
            ...
            this.debug(
              'error',
              `读取扇区地址不匹配: 期望[${expectedAddr.map((b) => b.toString(16).padStart(2, '0').toUpperCase()).join(' ')}], 实际[${responseAddr.map((b) => b.toString(16).padStart(2, '0').toUpperCase()).join(' ')}]`,
            )
          }
        }
      } else {
        this.debug(
          'error',
          `读取扇区响应格式错误: 头部匹配=${headerMatch}, 状态码=0x${response[10]?.toString(16).padStart(2, '0') || 'unknown'}`,
        )
      }
    } else {
  ...
  async protectFlash() {
    ...
    if (!this.compareRegisterValue(regVal, protectRegVal, mask)) {
      ...
      for (let i = 0; i < writeVal.length; i++) {
        ...
        this.debug(
          'debug',
          `计算写入值${i}: 目标=0x${protectRegVal[i].toString(16).padStart(2, '0')}, 反掩码=0x${invertedMask.toString(16).padStart(2, '0')}, 保留位=0x${preserved.toString(16).padStart(2, '0')}, 最终写入=0x${writeVal[i].toString(16).padStart(2, '0')}`,
        )
      }
      this.debug('debug', `计算写入值: [${writeVal.map((v) => '0x' + v.toString(16).padStart(2, '0')).join(', ')}]`)
