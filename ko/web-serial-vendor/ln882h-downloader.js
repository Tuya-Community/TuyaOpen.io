...
class LN882HDownloader extends BaseDownloader {
  constructor(serialPort, debugCallback) {
    ...
    this.RAM_BIN =
      typeof window !== 'undefined' && window.LN882H_RAM_BIN ? window.LN882H_RAM_BIN : new Uint8Array([0x00]) // 占位符，实际使用时需要加载完整数据
  }
  ...
  async sendData(data) {
    ...
    try {
      ...
      // 分包发送，防止 Mac 平台 USB 转串口桥接芯片 FIFO 溢出
      await this.pacedWrite(writer, data, this.baudrate || 115200)
    } finally {
      if (writer) {
        try {
          writer.releaseLock()
  ...
  async send(fileData, fileName, fileSize, packetSize = 128) {
    ...
    if (
      !(await this.sendFile(fileData, packetSize, () => {
        currentStep++
        if (this.onProgress) {
